...
coords <- matrix(c(-122.55, -122.35, 37.695, 37.85), byrow = TRUE, nrow = 2, ncol = 2, dimnames = list(c('x','y'),c('min','max'))) 
location <- coords %>% opq(timeout=50)
...
water <- location %>%
  add_osm_feature(key = "natural", 
                  value = c("water","lagoon","bay","beach","coastline")) %>%
  osmdata_sf()
rail <- location %>%
  add_osm_feature(key = "railway", 
...
#plot map
ggplot() + 
  geom_sf(data = ocean$osm_multipolygons, fill = '#c6e1e3') +
  geom_sf(data = ocean$osm_polygons, fill = '#c6e1e3') +
  geom_sf(data = water$osm_multipolygons, fill = '#c6e1e3') +
  geom_sf(data = water$osm_lines, fill = '#c6e1e3') +
...
#############################################
  ...
  geom_sf(data = chiwards2023,color="black",aes(fill = as.factor(councilvotes$LSD)),size=1,alpha=0.5)+
    scale_fill_manual(values=c("transparent","#459ED8"),labels=c("","Signed Letter"))+
    geom_sf_text(data=chiwards2023,aes(label = ward),size=3)+
    coord_sf(xlim=c(-87.95, -87.45),ylim=c(41.638, 42.07))+
    labs(title="Chicago Plan Commission",
      subtitle="June 2024 Residential Considerations")+
    theme_void()+
    labs(fill="")+
...
chibasemap+
  ...
  geom_sf_text(data=chiwards2023,aes(label = ward),size=3)+
  coord_sf(xlim=c(-87.75, -87.6),ylim=c(41.8, 42.02))+
