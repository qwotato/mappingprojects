...
westloopstops <- filter(ctaridepull,stationame == "Morgan-Lake" 
                 | stationame == "Ashland-Lake"
                 )

westloopstops$date <-  as.Date(westloopstops$month_beginning, format="%Y-%m-%d")
westloopstops$month <- format(as.Date(westloopstops$date, format="%Y-%m-%d"),"%m")
westloopstops$year <- format(as.Date(westloopstops$date, format="%Y-%m-%d"),"%Y")

westloopstops <- filter(westloopstops, year > 2013)

wloopback <- ggplot(westloopstops)+
  geom_line(aes(x=date,y=monthtotal,color=stationame),linetype=2,linewidth=1)+
  geom_smooth(data=filter(westloopstops,year>2020),aes(x=date,y=monthtotal,color=stationame,fill=stationame),alpha=0.05)+
  geom_smooth(data=filter(westloopstops,year<2020),aes(x=date,y=monthtotal,color=stationame,fill=stationame),alpha=0.05)+
  scale_color_manual(values=c("#e27ea6","#009b3a","#c60c30","#0b7aa4"))+
  scale_fill_manual(values=c("#e27ea6","#009b3a","#c60c30","#0b7aa4"))+
  guides(fill="none")+
  labs(x="Month",y="Ridership","Ridership",title="Chicago Transit Recovery",color="Mode:",subtitle="West Loop")+
...
ggsave(plot=wloopback, "C:/Users/arcannon/OneDrive - Burns & McDonnell/FE PE School/Rstudio2022/ILVIS/ctadata/plots/wloopback.jpeg",height=10,width=10)
...
#Looking at weekend vs weekday vs Sunday+Holiday

weekdaystrain <- ctaridepull
weekdaystrain$date <-  as.Date(weekdaystrain$month_beginning, format="%Y-%m-%d")
weekdaystrain$month <- format(as.Date(weekdaystrain$date, format="%Y-%m-%d"),"%m")
weekdaystrain$year <- format(as.Date(weekdaystrain$date, format="%Y-%m-%d"),"%Y")

weekdaystrain <- weekdaystrain %>% group_by(date) %>% summarise(Weekday=sum(avg_weekday_rides),Saturday=sum(avg_saturday_rides),Sunday_Holiday=sum(avg_sunday_holiday_rides))

weekdaystrain <- weekdaystrain %>% pivot_longer(!date,names_to = "dayofweek",values_to="rides")
weekdaystrain$month <- format(as.Date(weekdaystrain$date, format="%Y-%m-%d"),"%m")
weekdaystrain$year <- format(as.Date(weekdaystrain$date, format="%Y-%m-%d"),"%Y")

weekdaystrain <- filter(weekdaystrain, year > 2016)
weekdaystrain <- filter(weekdaystrain, year < 2025)

daytypetrain17_19 <- ggplot(weekdaystrain)+
  geom_col(data=filter(weekdaystrain,month==10),aes(x=date,y=225000),alpha=0.1,fill="red",width=30)+
  geom_line(aes(x=date,y=rides,color=dayofweek),linetype=1,linewidth=1)+
  scale_fill_manual(values=c("#e27ea6","#009b3a","#c60c30","#0b7aa4"))+
  guides(fill="none")+
  labs(x="Month",y="Ridership","Ridership",title="CTA Train Ridership",color="Mode:",subtitle="2017 - 2024")+
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10),label=scales::comma)+
  scale_x_date(date_breaks = "month", date_labels = "%Y-%m",expand=c(0,0))+
  theme_classic()+
  theme(plot.title = element_text(hjust = 0.5,face="bold"), plot.subtitle = element_text(hjust = 0.5),
        axis.text.y=element_text(color="black"), axis.text.x=element_text(color="black",angle=90,vjust=0.5),plot.caption = element_text( color="#404040"),
        panel.border = element_blank(),panel.grid.minor = element_blank(), legend.position="top",
...
ggsave(plot=daytypetrain17_19, "C:/Users/arcannon/OneDrive - Burns & McDonnell/FE PE School/Rstudio2022/ILVIS/ctadata/plots/daytypetrain17_24.jpeg",height=6,width=12)
...
colrecover <- ggplot(head(arrange(ctastopkeymixed,-recovery),17))+
