...
basemap <- ggplot() +
  geom_sf(data = lakemich,aes(fill=color,color=color))+
  geom_sf(data = cookcounty,fill="#fafbec",color="#eaeccf")+
  scale_fill_identity()+
  scale_color_identity()+
  #geom_sf(data = chiwards2023,aes(fill=Ward))+
  #geom_sf_text(data=warddata,aes(label = ward),size=2.5,vjust=1.5,alpha=0.9,hjust="left")+
 ...
 basemap +geom_sf(data = chiwards2023,aes(fill=Ward))+
   scale_fill_manual()+
   coord_sf(xlim=c(-87.96,-87.515),ylim=c(41.625,42.05),expand=FALSE)
...
votemap2 <- ggplot() +
  ...
  labs(fill="Support")+
  geom_sf_text(data=wardvotes,aes(label = Ward),size=3)+
  scale_fill_manual(values=c("white","#005e8b","red"),labels=c("",yesvotes[1,1]))+
  theme_void()+
  theme(legend.position=c(0.3,0.55))+
  theme(plot.title=element_text(hjust=0.1,vjust=-11.5))+
  theme(plot.subtitle=element_text(hjust=0.1,vjust=-15))+
